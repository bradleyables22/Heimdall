(function(global) {
  ...
  async function ensureBifrostSubscribeToken(topic) {
    ...
    const p = (async () => {
      try {
        ...
        const base = Heimdall.config.endpoints && Heimdall.config.endpoints.bifrostToken ? Heimdall.config.endpoints.bifrostToken : DEFAULT_BIFROST_TOKEN_ENDPOINT;
  ...
  function buildBifrostUrl(topic, st) {
    const base = Heimdall.config.endpoints && Heimdall.config.endpoints.bifrost ? Heimdall.config.endpoints.bifrost : DEFAULT_BIFROST_ENDPOINT;
    const url = new URL(base, global.location?.origin || void 0);
  ...
  function installSseSweeper() {
    ...
    const sweepIntervalMs = Heimdall.config.sseSweepIntervalMs || 5e3;
    setInterval(() => {
